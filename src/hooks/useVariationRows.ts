...
const getVariationTypes = (
  sourceVar?: VariationsMap,
  targetVar?: VariationsMap
): string[] => {
  const types = new Set([
    ...Object.keys(sourceVar || {}),
    ...Object.keys(targetVar || {})
  ]);
...
const processNestedVariations = (
  ...
  const nestedTypes = getVariationTypes(
    sourceValue?.variations,
    targetValue?.variations
  );
...
const processVariationLevel = (
  ...
  return Array.from(variations).flatMap((varKey) => {
    ...
    if (hasNestedVariations) {
      const nestedRows = processNestedVariations(
        sourceValue,
        targetValue,
        depth,
        currentPath
  ...
  const variationRows = useMemo(() => {
    ...
    return variationTypes.flatMap(variationType =>
      processVariationLevel(
        ...
        targetVariations?.[variationType],
        variationType
      )
    );
